# Puissance d'un nombre 1er dans une factorielle (Legendre) — transcription fidèle

> 🧾 **Manuscrit original :** `legendre-factorielle.pdf` (scan, 2 pages) · 📅 06/07/2020 · ✍️ KpihX
...
> 📄 **Source scannée :** [`legendre-factorielle.pdf`](legendre-factorielle.pdf) (restaurée depuis `/run/media/kpihx/KpihX-Datas1/Travaux/Documents/PDF/Puissance d'un nombre 1er dans une factorielle.pdf`, vérifiée `pdfinfo`, 2026-09-07)
...
De même les ($m$ nombres) $p^1, \dots, p^m$ sont les (seules) puissances (non unitaires) de $p$ à intervenir dans l'écriture $n! = 1 \times \dots \times n$. Or $p^m \leq n < p^{m+1}$ car $m = \mathrm{Max}(E)$. [ligne biffée au-dessus du « De plus » : premier jet « on $p$… $\implies n-p$… », illisible] De plus c-à-d $m\ln p \leq \ln n < (m+1)\ln p$, c-à-d $m \leq \frac{\ln n}{\ln p} < m+1$, c-à-d $m = E\left(\frac{\ln n}{\ln p}\right)$.
...
Aucune figure à reproduire : p. 1 vérifiée (texte + équations, encre bleue) ; p. 2 = suite textuelle sans figure d'après la transcription. Aucun script `reproduce_legendre_factorielle_N.py`, aucun PNG (aucun `assets/` créé).

---

## Vocabulaire

- $E(\cdot)$ = partie entière ; $Ona$ = on a ; $Mq$ = montrons que (ici implicite).
- $E$, $E_k^0$ = ensembles d'exposants/multiples ; $m = \mathrm{Max}(E)$, $m_k$ = cardinaux associés.
